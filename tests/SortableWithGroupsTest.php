...
class SortableWithGroupsTest extends TestCase
{
    ...
    {
        $newDummies = $this->newOneOfEachDummy();

        foreach ($newDummies as $newDummy) {
            $this->assertEquals($newDummy->buildSortQuery()->count(), $newDummy->getHighestOrderNumber());
        }
    }
    ...
    public function it_can_get_the_highest_order_number_with_trashed_models()
    {
        ...
        $newDummies = $this->newOneOfEachDummy(classType: DummyWithSoftDeletes::class);

        foreach ($newDummies as $newDummy) {
            $this->assertEquals($newDummy->buildSortQuery()->withTrashed()->count(), $newDummy->getHighestOrderNumber());
        }
    }
    ...
    public function it_provides_an_ordered_trait()
    {
        ...
        foreach ($newDummies as $dummy) {
            $i = 1;

            foreach ($dummy->buildSortQuery()->ordered()->get()->pluck('order_column') as $order) {
    ...
    public function it_will_not_fail_when_it_cant_move_the_order_down()
    {
        ...
        foreach ($newDummies as $dummy) {
            $lastModel = $dummy->buildSortQuery()->get()->last();

            $this->assertEquals($lastModel->order_column, 5);
    ...
    public function it_can_move_a_model_to_the_first_place()
    {
        ...
        foreach ($newDummies as $dummy) {
            ...
            $oldModels = $dummy->buildSortQuery()->whereNot('id', $positions[$i])->get();

            $model = Dummy::find($positions[$i]);
            ...
            $newModels = $dummy->buildSortQuery()->whereNot('id', $positions[$i])->get()->pluck('order_column', 'id');
    ...
    public function it_can_move_a_model_to_the_last_place()
    {
        ...
        foreach ($newDummies as $dummy) {
            ...
            $this->assertNotEquals(5, $model->order_column);
    ...
    public function it_can_use_config_properties()
    {
        config([
            ...
            'eloquent-sortable.sort_when_updating' => true,
            'eloquent-sortable.sort_when_deleting' => true,
        ]);
        ...
        $this->assertEquals(config('eloquent-sortable.sort_when_updating'), $model->shouldSortWhenUpdating());
        $this->assertEquals(config('eloquent-sortable.sort_when_deleting'), $model->shouldSortWhenDeleting());
    }
